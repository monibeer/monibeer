...
fkempresa int null,
constraint empresafuncionario foreign key (fkempresa)
...
fkempresa int,
constraint empresafrementadora foreign key (fkempresa) references empresa(idempresa),
fkcerveja int,
constraint fermentadoracerveja foreign key (fkcerveja) references cerveja(idcerveja)
);
...
insert into captura(sensor_analogico, dthora) values
(14.15, '2025-04-05 05:45:30'),
(10.23, '2025-02-05 06:10:15'),
(9.02,  '2025-03-10 14:45:32'),
(3.09,  '2025-01-05 08:00:00'),
...
-- (ajuste necessário: coluna representanteempresa não existe no seu schema atual)
-- select * from empresa where representanteempresa like '%_a';

-- atualizando os dados da tabela sensor: maquina atribuída não existe no schema atual, mas assumindo que queira adicionar:
alter table sensor add column maquinaatribuida varchar(40);
update sensor set maquinaatribuida = 'maquina 6' where idsensor = 2;

-- selecionando o tipo de cerveja da tabela fermentadora com case no estágio de fermentação
select tipocerveja, case 
    when estagiofermentacao = 'a' then 'fermentação inicial' 
    else 'em outras etapas' 
end as etapadefermentacao 
from fermentadora 
join cerveja on fermentadora.fkcerveja = cerveja.idcerveja;
...
-- (coluna representanteempresa não existe, então isso causaria erro)
-- select empresa.nomeempresa as nomedaempresa, empresa.representanteempresa as representante from empresa;

-- renomeando o campo da tabela captura (sensor_analogico) para (temperatura)
ALTER TABLE captura RENAME COLUMN sensor_analogico TO temperatura;
